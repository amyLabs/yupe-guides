...
Работа со статусами будет добавлена поздней, поэтому форма пока будет содержать только поле "Описание" 
```php
...
Если вы ни где не ошиблись, то можете попробовать добавить первую задачу. После её сохранения она появится в списке задач.

Изменение задачи по сути ни чем не отличается от ее создания, за исключением того, что нужно получить изменяемые данные. 
...
В первую очередь создаем фильтр, который будет запрещать обращение к методу `actionDelete` напрямую, а только через POST запрос:
